...
methods: {
  async func() {
    if (this.updatedData.title == "") alert(" You should fill the title");
    ...
    else if (this.stri == "" || this.stri[this.stri.length-1]== ',') alert(" Error in tags");
      ...
      try {
        ...
        this.updatedData.tags = this.stri.split(',').map(tag => tag.trim());
        this.updatedData.tags = this.updatedData.tags.filter(item => item !== '');
        this.updatedData.tags = this.updatedData.tags.filter(item => item !== ' ');
