  ...
  <div v-for="cv in resumes" :key="cv.id">
    <div v-if="cv.id === id" class="cv-container">
      <div class="cv-image-container">
        <router-link :to="{ name: 'Resume', params: { id: cv.id, templateName: cv.templateName } }">
          <img :src="getPreviewSrc(cv.templateName)" alt="CV Preview" class="cv-image">
        </router-link>
      </div>
      <div class="cv-details">
        ...
        <div class="cv-item">
          <div class="cv-city">
            <i class="fas fa-map-marker-alt"></i> {{ person.city }}
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...mapActions(['exportPDF']),
    getPreviewSrc(templateName) {
      return require(`../assets/resume-${templateName}.png`);
    },
    ...
    async duplicateCv(cv) {
      try {
        // Create a new CV object with the same data and a new ID
        const newCv = { ...cv, name: `${cv.name} (copy)` };
        delete newCv.id; // Remove the old ID
        const newDoc = await projectFirestore.collection('cv').add(newCv);
        
        // Add the new CV to the local resumes array
        this.resumes.push({ ...newCv, id: newDoc.id });
        this.$emit('update');
      } catch (err) {
    ...
    async installCv(cv) {
      ...
      this.$router.push({ name: 'CoolRtl', params: { id: cv.id } });
    },
    async deleteCv(cv) {
      try {
